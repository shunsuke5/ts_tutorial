...
console.log("\n配列要素へのアクセス");
{
    ...
    console.log("TypeScriptの要素の型");
    {
        ...
        const abc = ["a", "b", "c"]; // 返る値の型はstringとなる
...
console.log("\n読み取り専用の配列");
{
    ...
    const writableNumbers1 = readonlyNumbers; // OK
...
console.log("\n配列の共変性");
{
    let pochi = { isAnimal: true, isDog: true };
    let animal = pochi; // 代入OK
    // let pochi1: Dog = animal;   // エラー
    console.log(animal);
    console.log("配列は共変が許される");
    {
        const dogs = [pochi];
        const animals = dogs; // 代入OK
        console.log(animals);
        {
            const pochi = {
                isAnimal: true,
            ...
            const dogs = [pochi];
            const animals = dogs;
            console.log(dogs[0]); // { isAnimal: true, wanwan: [Function: wanwan] }
            animals[0] = { isAnimal: true }; // 同時にdogs[0]も書き換わる
            console.log(dogs[0]); // { isAnimal: true }
            const mayBePochi = dogs[0];
            // console.log(mayBePochi.wanwan());   // JS実行時エラー「TypeError: mayBePochi.wanwan is not a function」
        }
    }
}
